...
function Dashboard({ token, onLogout }: { token: string; onLogout: () => void }) {
  ...
  // Fetch feature flags for progressive rollout
  const { data: features } = useQuery<Features>({
    queryKey: ['features'],
    queryFn: async () => {
      const response = await axios.get(`${API_URL}/dashboard/features`, axiosConfig);
  ...
  return (
    <div className="dashboard">
      ...
      {/* 🎯 CANARY FEATURE: Flash Sale Banner (Only for Tenant 2) */}
      {features?.flashSaleBanner && (
        <div style={{
          ...
          <h2 style={{ margin: '0 0 10px 0', fontSize: '24px' }}>🚀 BLUE/GREEN TEST: You're on GREEN! 🚀</h2>
          <p style={{ margin: '0', fontSize: '16px' }}>
            This is the CANARY version! If you see this, Blue/Green deployment works!
          </p>
          <p style={{ margin: '10px 0 0 0', fontSize: '14px', opacity: 0.9 }}>
            <strong>🟢 GREEN Deployment</strong> - Tenant {features.tenantId} (Testing Phase)
          </p>
        </div>
      )}
